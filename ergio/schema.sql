...
-- ========================================
-- NEW TABLES - ERGIO v3 (Open-Source Engines)
-- ========================================
...
-- 19. SEO PAGES (generated by Local Discovery engine)
...
-- 22. ENGINE LOGS (monitor all 4 acquisition engines)
...
-- 23. PLATFORM ANALYTICS (for the separate analytics dashboard)
...
CREATE POLICY "Users can view own SEO pages" ON public.seo_pages FOR SELECT USING (business_id IN (SELECT id FROM public.businesses WHERE user_id = auth.uid()));
CREATE POLICY "Users can insert own SEO pages" ON public.seo_pages FOR INSERT WITH CHECK (business_id IN (SELECT id FROM public.businesses WHERE user_id = auth.uid()));
CREATE POLICY "Users can update own SEO pages" ON public.seo_pages FOR UPDATE USING (business_id IN (SELECT id FROM public.businesses WHERE user_id = auth.uid()));
...
CREATE POLICY "Users can view own engine logs" ON public.engine_logs FOR SELECT USING (business_id IN (SELECT id FROM public.businesses WHERE user_id = auth.uid()));
...
CREATE INDEX IF NOT EXISTS idx_engine_logs_business ON public.engine_logs(business_id);
CREATE INDEX IF NOT EXISTS idx_engine_logs_engine ON public.engine_logs(engine_name);
CREATE INDEX IF NOT EXISTS idx_platform_analytics_type ON public.platform_analytics(event_type);
CREATE INDEX IF NOT EXISTS idx_platform_analytics_date ON public.platform_analytics(recorded_at);
...
CREATE TRIGGER update_seo_pages_modtime BEFORE UPDATE ON public.seo_pages FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();
CREATE TRIGGER update_rewards_modtime BEFORE UPDATE ON public.rewards FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();
