...
async function execAnalyze(b) {
  ...
  const summary = await callGroq([{ role: 'user', content: `You are a sharp business growth consultant. Business: "${b.name}" (${b.type || b.industry || ''}, ${b.city || 'Nigeria'}). Website title: "${title}". Description: "${desc}". Contact email(s): ${emails.join(', ') || 'none found'}. Has WhatsApp button: ${hasWA}. Has online payments: ${hasPay}. Has booking: ${hasBooking}. In under 120 words: (1) one-sentence verdict on their web presence, (2) the 3 fastest fixes to help them MAKE MORE MONEY. Be direct, no fluff.` }], { maxTokens: 300 }).catch(() => 'Analysis complete — see raw data.');
...
async function execFindLeads(b) {
  ...
  if (leads.length === 0) {
    fallback = true;
    const txt = await callGroq([{ role: 'user', content: `List 6 realistic high-intent lead profiles for a Nigerian ${industry || 'service'} business in ${city}: name, business type, why they need "${b.name}"'s services. JSON array, fields: name, type, why. No markdown.` }], { maxTokens: 500 }).catch(() => '[]');
...
async function execOutreach(b, goal) {
  const txt = await callGroq([{ role: 'user', content: `Write 2 cold outreach messages for "${b.name}" (a ${b.type || b.industry || 'business'} in ${b.city || 'Nigeria'}). Goal: ${goal || 'get more paying customers'}. (1) A WhatsApp message under 400 characters, friendly, Nigerian tone, ends with a clear question. (2) A cold email with subject line, under 120 words. Return as: WHATSAPP: ... then EMAIL SUBJECT: ... then EMAIL: ...` }], { maxTokens: 400 }).catch(() => '');
  if (!txt.trim()) return { ok: false, note: 'AI writer unavailable right now.' };
...
async function execSocial(b, goal) {
  const txt = await callGroq([{ role: 'user', content: `Create a 5-post social media kit for "${b.name}" (${b.type || 'business'}, ${b.city || 'Nigeria'}). Goal: ${goal || 'attract customers'}. For each post: PLATFORM: (Instagram or WhatsApp Status or Twitter/X), HOOK: one scroll-stopping line, BODY: 2-3 short lines, CTA: one line. Nigerian audience, natural tone, no hashtag spam (max 3).` }], { maxTokens: 600 }).catch(() => '');
  return { ok: !!txt.trim(), posts: txt.slice(0, 2500) || 'AI unavailable.' };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'goal') {
      ...
      const strategy = await callGroqFast([{ role: 'user', content: `Business: "${b.name}" (${b.type || ''}, ${b.city || 'Nigeria'}). User's goal: "${goal}". Available actions: ${plan.map(s => STEP_LABELS[s]).join('; ')}. In one sentence, explain the strategy for this goal. Under 30 words.` }]).catch(() => '');
      return success(res, { success: true, business: { id: b.id, name: b.name }, goal, strategy: strategy.trim() || 'Plan ready. Run the steps below.', plan, step_labels: STEP_LABELS });
    }
