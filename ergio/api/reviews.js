...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET' && action === 'list') {
      ...
      if (error) return res.status(200).json({ reviews: [], summary: { total: 0, avg_rating: 0 } });
      const reviews = (data || []).filter(r => r.status === 'approved');
      const avgRating = reviews.length ? (reviews.reduce((s, r) => s + r.rating, 0) / reviews.length).toFixed(1) : 0;
      return res.status(200).json({ reviews, summary: { total: reviews.length, avg_rating: parseFloat(avgRating) } });
    }
    ...
    if (req.method === 'POST' && action === 'create') {
      ...
      const { data, error } = await sb.from('reviews').insert({
        ...
        rating: Math.min(5, Math.max(1, parseInt(rating))), text, booking_id, status: 'pending'
      }).select();
    ...
    if (req.method === 'POST' && action === 'request') {
      ...
      const waMessage = `Hi ${client_name}! We'd love your feedback. Rate us here: ${reviewLink}`;
    ...
    return res.status(200).json({ reviews: [], summary: { total: 0, avg_rating: 0 } });
  } catch (err) {
    return res.status(200).json({ reviews: [], summary: { total: 0, avg_rating: 0 }, error: err.message });
  }
}
